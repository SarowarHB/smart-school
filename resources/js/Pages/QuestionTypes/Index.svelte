<script>
  ...
  function doDelete() {
    ...
    router.delete(`/question-types/${deleteTarget.id}`, { onSuccess: () => { deleteTarget = null; } });
  }

  let modalMode   = $state(null);
  let editingItem = $state(null);
  const form = useForm({ id: '', name: '', description: '' });

  function openCreate() { form.reset(); form.clearErrors(); modalMode = 'create'; editingItem = null; }
  function openEdit(item) { form.id = item.id; form.name = item.name; form.description = item.description ?? ''; form.clearErrors(); modalMode = 'edit'; editingItem = item; }
  function closeModal() { form.reset(); form.clearErrors(); modalMode = null; editingItem = null; }
  function submitForm(e) {
    e.preventDefault();
    if (modalMode === 'create') form.post('/question-types', { onSuccess: () => closeModal() });
    else if (modalMode === 'edit' && editingItem) form.put(`/question-types/${editingItem.id}`, { onSuccess: () => closeModal() });
  }
</script>

<svelte:head><title>Question Types | SvelteAdmin</title></svelte:head>

<AppLayout>
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Question Types</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{types.total} record{types.total === 1 ? '' : 's'} total</p>
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-16">ID</th>
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
          {#each types.data as item}
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
              <td class="px-6 py-4 text-gray-400 dark:text-gray-500 font-mono text-xs">{item.id}</td>
          ...
          {:else}
            <tr><td colspan="4" class="px-6 py-16 text-center text-gray-400 dark:text-gray-500">No types found. <button onclick={openCreate} class="text-primary-500 hover:underline">Create one.</button></td></tr>
          {/each}
        </tbody>
      </table>
    </div>
    {#if types.last_page > 1}
      <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-800">
        <p class="text-xs text-gray-500 dark:text-gray-400">Showing {types.from}–{types.to} of {types.total}</p>
        <div class="flex gap-1">
          {#each types.links as link}
            {#if link.url}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 border border-gray-200 dark:border-gray-700 z-10">
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 class="font-semibold text-gray-900 dark:text-white">Delete Type</h3>
...
{#if modalMode}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 border border-gray-200 dark:border-gray-700 z-10">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-5">{modalMode === 'create' ? 'Create Type' : 'Edit Type'}</h3>
      <form onsubmit={submitForm} novalidate class="space-y-4">
        {#if modalMode === 'create'}
          <div>
            <label for="modal-id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">ID <span class="text-red-500">*</span></label>
            <input id="modal-id" type="number" bind:value={form.id} placeholder="Unique numeric ID" min="1" class="w-full px-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500/40 {form.errors.id ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-gray-700 focus:border-primary-500'}" />
            {#if form.errors.id}<p class="mt-1.5 text-xs text-red-500">{form.errors.id}</p>{/if}
          </div>
        {/if}
        <div>
          ...
          <input id="modal-name" type="text" bind:value={form.name} placeholder="Type name" class="w-full px-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500/40 {form.errors.name ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-gray-700 focus:border-primary-500'}" />
