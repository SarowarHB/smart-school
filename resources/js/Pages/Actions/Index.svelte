<script>
  ...
  function confirmDelete(action) { deleteTarget = action; }
  function cancelDelete()        { deleteTarget = null; }
  function doDelete() {
    if (!deleteTarget) return;
    router.delete(`/actions/${deleteTarget.id}`, {
  ...
  function submitForm(e) {
    e.preventDefault();
    if (modalMode === 'create') {
      form.post('/actions', { onSuccess: () => closeModal() });
    } else if (modalMode === 'edit' && editingAction) {
      form.put(`/actions/${editingAction.id}`, { onSuccess: () => closeModal() });
    }
  }
</script>

<svelte:head>
  <title>Actions | SvelteAdmin</title>
</svelte:head>
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
        {actions.total} action{actions.total === 1 ? '' : 's'} total
      </p>
    </div>
    <button
      onclick={openCreate}
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
          {#each actions.data as action, index}
            ...
            <tr>
              <td colspan="3" class="px-6 py-16 text-center text-gray-400 dark:text-gray-500">
                <div class="flex flex-col items-center gap-2">
                  ...
                  No actions found. <button onclick={openCreate} class="text-primary-500 hover:underline">Create one.</button>
                </div>
              </td>
            </tr>
          {/each}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6
      ...
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 class="font-semibold text-gray-900 dark:text-white">Delete Action</h3>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6
      ...
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-5">
        {modalMode === 'create' ? 'Create Action' : 'Edit Action'}
      </h3>

      <form onsubmit={submitForm} novalidate>
        <div class="mb-5">
          <label for="modal-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Action Name <span class="text-red-500" aria-hidden="true">*</span>
          </label>
          <input
            id="modal-name"
            type="text"
        ...
        <div class="flex gap-3 pt-2">
          ...
          >
            ...
            {/if}
            {modalMode === 'create' ? 'Create Action' : 'Save Changes'}
          </button>
        </div>
      </form>
    </div>
