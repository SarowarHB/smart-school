<script>
  ...
  let { assessmentResources, assessments } = $props();
  ...
  function doDelete() {
    ...
    router.delete(`/assessment-resources/${deleteTarget.id}`, {
  ...
  function openEdit(item) {
    form.assessment_id = item.assessment_id ?? '';
    form.resource_type = item.resource_type ?? 'document';
    form.url           = item.url ?? '';
    form.question_id   = item.question_id ?? '';
    form.resource      = item.resource ? JSON.stringify(item.resource, null, 2) : '';
    form.clearErrors();
    modalMode   = 'edit';
    editingItem = item;
  }
  ...
  function submitForm(e) {
    e.preventDefault();
    if (modalMode === 'create') {
      form.post('/assessment-resources', { onSuccess: () => closeModal() });
    } else if (modalMode === 'edit' && editingItem) {
      form.put(`/assessment-resources/${editingItem.id}`, { onSuccess: () => closeModal() });
    }
  }

  const resourceTypes = ['document', 'video', 'audio', 'image', 'link', 'other'];
  const jsonPlaceholder = '{"key": "value"}';
</script>

<svelte:head>
  <title>Assessment Resources | SvelteAdmin</title>
</svelte:head>
...
<AppLayout>
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Assessment Resources</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
        {assessmentResources.total} record{assessmentResources.total === 1 ? '' : 's'} total
      </p>
    </div>
    <button
      onclick={openCreate}
  ...
  <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
            ...
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Assessment</th>
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Type</th>
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">URL</th>
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Question ID</th>
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
          {#each assessmentResources.data as item, index}
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors group">
              ...
              <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{item.assessment_title ?? '—'}</td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4 text-gray-600 dark:text-gray-400 max-w-xs truncate">
                {#if item.url}
                  <a href={item.url} target="_blank" rel="noopener noreferrer"
                     class="text-primary-600 dark:text-primary-400 hover:underline truncate block max-w-[200px]">
                    {item.url}
                  </a>
                {:else}
                  —
    ...
    <!-- Pagination -->
    {#if assessmentResources.last_page > 1}
      <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-800">
        <p class="text-xs text-gray-500 dark:text-gray-400">
          Showing {assessmentResources.from}–{assessmentResources.to} of {assessmentResources.total}
        </p>
        <div class="flex gap-1">
          {#each assessmentResources.links as link}
            {#if link.url}
              <button
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6
      ...
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 class="font-semibold text-gray-900 dark:text-white">Delete Resource</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400">This action cannot be undone.</p>
        </div>
      </div>
      <p class="text-sm text-gray-700 dark:text-gray-300 mb-6">
        Are you sure you want to delete this assessment resource?
      </p>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8
      ...
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-5">
        {modalMode === 'create' ? 'Create Assessment Resource' : 'Edit Assessment Resource'}
      </h3>
      ...
      <form onsubmit={submitForm} novalidate class="space-y-4">
        ...
        <!-- Assessment -->
        <div>
          <label for="modal-assessment" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Assessment <span class="text-red-500" aria-hidden="true">*</span>
          </label>
          <select
            id="modal-assessment"
            bind:value={form.assessment_id}
          ...
          {#if form.errors.assessment_id}
            <p class="mt-1.5 text-xs text-red-500">{form.errors.assessment_id}</p>
          {/if}
        </div>

        <!-- Resource Type -->
        <div>
          <label for="modal-resource-type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Resource Type <span class="text-red-500" aria-hidden="true">*</span>
          </label>
          <select
            id="modal-resource-type"
            bind:value={form.resource_type}
            class="w-full px-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-800
                   text-gray-900 dark:text-white transition-colors focus:outline-none
                   focus:ring-2 focus:ring-primary-500/40
          ...
          {#if form.errors.resource_type}
            <p class="mt-1.5 text-xs text-red-500">{form.errors.resource_type}</p>
          {/if}
        </div>

        <!-- URL -->
        <div>
          <label for="modal-url" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            URL
          </label>
          <input
            id="modal-url"
          ...
          />
          {#if form.errors.url}
            <p class="mt-1.5 text-xs text-red-500">{form.errors.url}</p>
          {/if}
        </div>

        <!-- Question ID -->
        <div>
          <label for="modal-question-id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Question ID
          </label>
          <input
            ...
            class="w-full px-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-800
                   ...
                   {form.errors.question_id
                     ...
                     : 'border-gray-200 dark:border-gray-700 focus:border-primary-500 dark:focus:border-primary-500'}"
          />
          {#if form.errors.question_id}
            <p class="mt-1.5 text-xs text-red-500">{form.errors.question_id}</p>
          {/if}
        </div>
        ...
        <div>
          <label for="modal-resource" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Resource <span class="text-xs text-gray-400 font-normal">(JSON)</span>
          </label>
          <textarea
            id="modal-resource"
            ...
            class="w-full px-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-800
                   text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500
                   font-mono transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500/40
                   resize-none
                   {form.errors.resource
                     ? 'border-red-400 dark:border-red-500'
                     : 'border-gray-200 dark:border-gray-700 focus:border-primary-500 dark:focus:border-primary-500'}"
          ></textarea>
          {#if form.errors.resource}
            <p class="mt-1.5 text-xs text-red-500">{form.errors.resource}</p>
          {/if}
        </div>
        ...
        <div class="flex gap-3 pt-2">
          ...
          >
            ...
            {/if}
            {modalMode === 'create' ? 'Create Resource' : 'Save Changes'}
          </button>
        </div>
      </form>
    </div>
