<script>
  ...
  let { policies, roles, actions } = $props();

  // ── Delete ─────────────────────────────────────────────────────────────────
  let deleteTarget = $state(null);

  function confirmDelete(policy) { deleteTarget = policy; }
  function cancelDelete()        { deleteTarget = null; }
  function doDelete() {
    if (!deleteTarget) return;
    router.delete(`/policies/${deleteTarget.id}`, {
  ...
  const form = useForm({ name: '', description: '', role_id: '', action_id: '' });
  ...
  function openEdit(policy) {
    ...
    form.role_id     = policy.role_id ?? '';
    form.action_id   = policy.action_id ?? '';
    form.clearErrors();
    modalMode     = 'edit';
  ...
  function submitForm(e) {
    e.preventDefault();
    if (modalMode === 'create') {
      form.post('/policies', { onSuccess: () => closeModal() });
    } else if (modalMode === 'edit' && editingPolicy) {
      form.put(`/policies/${editingPolicy.id}`, { onSuccess: () => closeModal() });
    }
  }
</script>

<svelte:head>
  <title>Policies | SvelteAdmin</title>
</svelte:head>
...
<AppLayout>
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Policies</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
        {policies.total} polic{policies.total === 1 ? 'y' : 'ies'} total
      </p>
    </div>
    <button
      onclick={openCreate}
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
            ...
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Action</th>
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
          {#each policies.data as policy, index}
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors group">
              ...
              <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{policy.name ?? '—'}</td>
              <td class="px-6 py-4 text-gray-600 dark:text-gray-400 max-w-xs truncate">{policy.description ?? '—'}</td>
              <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{policy.role_name ?? '—'}</td>
              <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{policy.action_name ?? '—'}</td>
              <td class="px-6 py-4">
            ...
            <tr>
              <td colspan="6" class="px-6 py-16 text-center text-gray-400 dark:text-gray-500">
                <div class="flex flex-col items-center gap-2">
                  ...
                  No policies found. <button onclick={openCreate} class="text-primary-500 hover:underline">Create one.</button>
                </div>
              </td>
            </tr>
          {/each}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6
      ...
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 class="font-semibold text-gray-900 dark:text-white">Delete Policy</h3>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8
      ...
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-5">
        {modalMode === 'create' ? 'Create Policy' : 'Edit Policy'}
      </h3>
      ...
      <form onsubmit={submitForm} novalidate class="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Read Only, Full Access"
        ...
        <!-- Description -->
        <div>
          <label for="modal-description" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Description
          </label>
          <input
          ...
          {#if form.errors.description}
            <p class="mt-1.5 text-xs text-red-500">{form.errors.description}</p>
          {/if}
        </div>
        ...
        <!-- Action -->
        <div>
          <label for="modal-action" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Action
          </label>
          <select
            id="modal-action"
          ...
          {#if form.errors.action_id}
            <p class="mt-1.5 text-xs text-red-500">{form.errors.action_id}</p>
          {/if}
        </div>
