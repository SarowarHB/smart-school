<script>
  ...
  function doDelete() {
    ...
    router.delete(`/question-subjects/${deleteTarget.id}`, { onSuccess: () => { deleteTarget = null; } });
  }
  ...
  function submitForm(e) {
    e.preventDefault();
    if (modalMode === 'create') form.post('/question-subjects', { onSuccess: () => closeModal() });
    else if (modalMode === 'edit' && editingItem) form.put(`/question-subjects/${editingItem.id}`, { onSuccess: () => closeModal() });
  }
</script>

<svelte:head><title>Question Subjects | SvelteAdmin</title></svelte:head>

<AppLayout>
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Question Subjects</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{subjects.total} record{subjects.total === 1 ? '' : 's'} total</p>
    </div>
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
          {#each subjects.data as item, index}
          ...
          {:else}
            <tr><td colspan="4" class="px-6 py-16 text-center text-gray-400 dark:text-gray-500">No subjects found. <button onclick={openCreate} class="text-primary-500 hover:underline">Create one.</button></td></tr>
          {/each}
        </tbody>
      </table>
    </div>
    {#if subjects.last_page > 1}
      <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-800">
        <p class="text-xs text-gray-500 dark:text-gray-400">Showing {subjects.from}–{subjects.to} of {subjects.total}</p>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 border border-gray-200 dark:border-gray-700 z-10">
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 class="font-semibold text-gray-900 dark:text-white">Delete Subject</h3>
...
{#if modalMode}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 border border-gray-200 dark:border-gray-700 z-10">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-5">{modalMode === 'create' ? 'Create Subject' : 'Edit Subject'}</h3>
      <form onsubmit={submitForm} novalidate class="space-y-4">
        <div>
          ...
          <input id="modal-name" type="text" bind:value={form.name} placeholder="Subject name" class="w-full px-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500/40 {form.errors.name ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-gray-700 focus:border-primary-500'}" />
