<script>
  ...
  function confirmDelete(resource) { deleteTarget = resource; }
  function cancelDelete()          { deleteTarget = null; }
  function doDelete() {
    if (!deleteTarget) return;
    router.delete(`/resources/${deleteTarget.id}`, {
  ...
  const form = useForm({ name: '', description: '', res_url: '' });
  ...
  function submitForm(e) {
    e.preventDefault();
    if (modalMode === 'create') {
      form.post('/resources', { onSuccess: () => closeModal() });
    } else if (modalMode === 'edit' && editingResource) {
      form.put(`/resources/${editingResource.id}`, { onSuccess: () => closeModal() });
    }
  }
</script>

<svelte:head>
  <title>Resources | SvelteAdmin</title>
</svelte:head>
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
          {#each resources.data as resource, index}
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors group">
              ...
              <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{resource.name ?? '—'}</td>
              <td class="px-6 py-4 text-gray-600 dark:text-gray-400 max-w-xs truncate">{resource.description ?? '—'}</td>
              <td class="px-6 py-4 text-gray-600 dark:text-gray-400">
                {#if resource.res_url}
                  <a href={resource.res_url} target="_blank" rel="noopener noreferrer"
            ...
            <tr>
              <td colspan="5" class="px-6 py-16 text-center text-gray-400 dark:text-gray-500">
                <div class="flex flex-col items-center gap-2">
                  ...
                  No resources found. <button onclick={openCreate} class="text-primary-500 hover:underline">Create one.</button>
                </div>
              </td>
            </tr>
          {/each}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8
      ...
      <form onsubmit={submitForm} novalidate class="space-y-4">
        ...
        <!-- URL -->
        <div>
          <label for="modal-res-url" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            URL
          </label>
          <input
            id="modal-res-url"
            type="text"
            bind:value={form.res_url}
            ...
            class="w-full px-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-800
                   ...
                   {form.errors.res_url
                     ...
                     : 'border-gray-200 dark:border-gray-700 focus:border-primary-500 dark:focus:border-primary-500'}"
          />
          {#if form.errors.res_url}
            <p class="mt-1.5 text-xs text-red-500">{form.errors.res_url}</p>
          {/if}
        </div>
